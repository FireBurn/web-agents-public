...
static void parse_config_value(am_xml_parser_ctx_t *x, const char *prm, int type, int *itm_sz, void *itm,
        ...
        *value = strndup(val, len);
        if (x->log_enable) {
            if (strstr(prm, "password") != NULL || strstr(prm, "key") != NULL) {
                AM_LOG_DEBUG(x->conf->instance_id, "am_parse_config_xml() %s is set to '********'", prm);
                break;
            }
            AM_LOG_DEBUG(x->conf->instance_id, "am_parse_config_xml() %s is set to '%s'", prm, LOGEMPTY(*value));
        }
    } break;
    case CONF_NUMBER: {
        int *value = (int *)itm;
    ...
    case CONF_STRING_LIST: {
        ...
        *value = value_tmp;
        (*value)[old_sz] = v;
        if (x->log_enable) {
            AM_LOG_DEBUG(x->conf->instance_id, "am_parse_config_xml() %s is set to %d value(s)", prm, *itm_sz);
        }
    } break;
...
static void parse_other_options(am_xml_parser_ctx_t *ctx, const char *val, int len) {
    ...
    parse_config_value(ctx, AM_AGENTS_CONFIG_LOGIN_URL_MAP, CONF_STRING_MAP, &ctx->conf->login_url_sz,
                       &ctx->conf->login_url, val, len);
    ...
    parse_config_value(ctx, AM_AGENTS_CONFIG_CMP_CASE_IGNORE, CONF_NUMBER, NULL, &ctx->conf->url_eval_case_ignore, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_POLICY_CACHE_VALID, CONF_NUMBER, NULL, &ctx->conf->policy_cache_valid, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_TOKEN_CACHE_VALID, CONF_NUMBER, NULL, &ctx->conf->token_cache_valid, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_UID_PARAM, CONF_STRING, NULL, &ctx->conf->userid_param, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_UID_PARAM_TYPE, CONF_STRING, NULL, &ctx->conf->userid_param_type, val,
                       len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_PROFILE_MODE, CONF_ATTR_MODE, NULL, &ctx->conf->profile_attr_fetch,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_PROFILE_MAP, CONF_STRING_MAP, &ctx->conf->profile_attr_map_sz,
                       &ctx->conf->profile_attr_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_SESSION_MODE, CONF_ATTR_MODE, NULL, &ctx->conf->session_attr_fetch,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_SESSION_MAP, CONF_STRING_MAP, &ctx->conf->session_attr_map_sz,
                       &ctx->conf->session_attr_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_RESPONSE_MODE, CONF_ATTR_MODE, NULL, &ctx->conf->response_attr_fetch,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_RESPONSE_MAP, CONF_STRING_MAP, &ctx->conf->response_attr_map_sz,
                       &ctx->conf->response_attr_map, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_SSO_ONLY, CONF_NUMBER, NULL, &ctx->conf->sso_only, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ACCESS_DENIED_URL, CONF_STRING, NULL, &ctx->conf->access_denied_url, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_FQDN_CHECK_ENABLE, CONF_NUMBER, NULL, &ctx->conf->fqdn_check_enable, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_FQDN_DEFAULT, CONF_STRING, NULL, &ctx->conf->fqdn_default, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_FQDN_MAP, CONF_STRING_MAP, &ctx->conf->fqdn_map_sz, &ctx->conf->fqdn_map,
                       val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_COOKIE_RESET_ENABLE, CONF_NUMBER, NULL, &ctx->conf->cookie_reset_enable,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_COOKIE_RESET_MAP, CONF_STRING_MAP, &ctx->conf->cookie_reset_map_sz,
                       &ctx->conf->cookie_reset_map, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_NOT_ENFORCED_URL, CONF_STRING_MAP, &ctx->conf->not_enforced_map_sz,
                       &ctx->conf->not_enforced_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_NOT_ENFORCED_INVERT, CONF_NUMBER, NULL, &ctx->conf->not_enforced_invert,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_NOT_ENFORCED_ATTR, CONF_NUMBER, NULL, &ctx->conf->not_enforced_fetch_attr,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_NOT_ENFORCED_IP, CONF_STRING_MAP, &ctx->conf->not_enforced_ip_map_sz,
                       &ctx->conf->not_enforced_ip_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_EXT_NOT_ENFORCED_URL, CONF_STRING_MAP, &ctx->conf->not_enforced_ext_map_sz,
                       &ctx->conf->not_enforced_ext_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_NOT_ENFORCED_REGEX_ENABLE, CONF_NUMBER, NULL,
                       &ctx->conf->not_enforced_regex_enable, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_EXT_NOT_ENFORCED_REGEX_ENABLE, CONF_NUMBER, NULL,
                       &ctx->conf->not_enforced_ext_regex_enable, val, len);
    ...
    parse_config_value(ctx, AM_AGENTS_CONFIG_CLIENT_IP_VALIDATE, CONF_NUMBER, NULL, &ctx->conf->client_ip_validate, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_COOKIE_PREFIX, CONF_STRING, NULL, &ctx->conf->cookie_prefix, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ATTR_COOKIE_MAX_AGE, CONF_NUMBER, NULL, &ctx->conf->cookie_maxage, val,
                       len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_CDSSO_ENABLE, CONF_NUMBER, NULL, &ctx->conf->cdsso_enable, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_CDSSO_LOGIN, CONF_STRING_MAP, &ctx->conf->cdsso_login_map_sz,
                       &ctx->conf->cdsso_login_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_CDSSO_DOMAIN, CONF_STRING_MAP, &ctx->conf->cdsso_cookie_domain_map_sz,
                       &ctx->conf->cdsso_cookie_domain_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_CDSSO_DENY_CLEANUP_DISABLE, CONF_NUMBER, NULL,
                       &ctx->conf->cdsso_deny_cleanup_disable, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_LOGOUT_URL, CONF_STRING_MAP, &ctx->conf->openam_logout_map_sz,
                       &ctx->conf->openam_logout_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_APP_LOGOUT_URL, CONF_STRING_MAP, &ctx->conf->logout_map_sz,
                       &ctx->conf->logout_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_LOGOUT_REDIRECT_URL, CONF_STRING, NULL, &ctx->conf->logout_redirect_url,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_LOGOUT_COOKIE_RESET, CONF_STRING_MAP,
                       &ctx->conf->logout_cookie_reset_map_sz, &ctx->conf->logout_cookie_reset_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_LOGOUT_REGEX_ENABLE, CONF_NUMBER, NULL, &ctx->conf->logout_regex_enable,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_LOGOUT_URL_REGEX, CONF_STRING, NULL, &ctx->conf->logout_url_regex, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_LOGOUT_REDIRECT_DISABLE, CONF_NUMBER, NULL,
                       &ctx->conf->logout_redirect_disable, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_POLICY_SCOPE, CONF_NUMBER, NULL, &ctx->conf->policy_scope_subtree, val,
                       len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_RESOLVE_CLIENT_HOST, CONF_NUMBER, NULL, &ctx->conf->resolve_client_host,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_POLICY_ENCODE_SPECIAL_CHAR, CONF_NUMBER, NULL,
                       &ctx->conf->policy_eval_encode_chars, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_COOKIE_ENCODE_SPECIAL_CHAR, CONF_NUMBER, NULL,
                       &ctx->conf->cookie_encode_chars, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_OVERRIDE_PROTO, CONF_NUMBER, NULL, &ctx->conf->override_protocol, val,
                       len);
    ...
    parse_config_value(ctx, AM_AGENTS_CONFIG_OVERRIDE_NOTIFICATION_URL, CONF_NUMBER, NULL,
                       &ctx->conf->override_notif_url, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_VALID, CONF_NUMBER, NULL, &ctx->conf->config_valid, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_PASSWORD_REPLAY_KEY, CONF_STRING, NULL, &ctx->conf->password_replay_key,
                       val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_POLICY_CLOCK_SKEW, CONF_NUMBER, NULL, &ctx->conf->policy_clock_skew, val,
                       len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_GOTO_PARAM_NAME, CONF_STRING, NULL, &ctx->conf->url_redirect_param, val,
                       len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_CACHE_CONTROL_ENABLE, CONF_NUMBER, NULL, &ctx->conf->cache_control_enable,
                       val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_USE_REDIRECT_ADVICE, CONF_NUMBER, NULL,
                       &ctx->conf->use_redirect_for_advice, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_CLIENT_IP_HEADER, CONF_STRING, NULL, &ctx->conf->client_ip_header, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_CLIENT_HOSTNAME_HEADER, CONF_STRING, NULL,
                       &ctx->conf->client_hostname_header, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_INVALID_URL, CONF_STRING, NULL, &ctx->conf->url_check_regex, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_CONDITIONAL_LOGIN_URL, CONF_STRING_MAP, &ctx->conf->cond_login_url_sz,
                       &ctx->conf->cond_login_url, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_COOKIE_HTTP_ONLY, CONF_NUMBER, NULL, &ctx->conf->cookie_http_only, val,
                       len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_MULTI_VALUE_SEPARATOR, CONF_STRING, NULL, &ctx->conf->multi_attr_separator,
                       val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_IIS_LOGON_USER, CONF_NUMBER, NULL, &ctx->conf->logon_user_enable, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_IIS_PASSWORD_HEADER, CONF_NUMBER, NULL, &ctx->conf->password_header_enable,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_PDP_JS_REPOST, CONF_NUMBER, NULL, &ctx->conf->pdp_js_repost, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_JSON_URL, CONF_STRING_MAP, &ctx->conf->json_url_map_sz,
                       &ctx->conf->json_url_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_JSON_URL_INVERT, CONF_NUMBER, NULL, &ctx->conf->json_url_invert, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_JSON_HEADER, CONF_STRING_MAP, &ctx->conf->json_header_map_sz,
                       &ctx->conf->json_header_map, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_JSON_RESPONSE_CODE, CONF_NUMBER, NULL, &ctx->conf->json_url_response_code,
                       val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_AUDIT_REMOTE_INTERVAL, CONF_NUMBER, NULL,
                       &ctx->conf->audit_remote_interval, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_AUDIT_REMOTE_FILE, CONF_STRING, NULL, &ctx->conf->audit_file_remote, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_AUDIT_DISPOSITION, CONF_STRING, NULL, &ctx->conf->audit_file_disposition,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_AUDIT_LEVEL, CONF_AUDIT_LEVEL, NULL, &ctx->conf->audit_level, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_ANONYMOUS_USER_ENABLE, CONF_NUMBER, NULL,
                       &ctx->conf->anon_remote_user_enable, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_ANONYMOUS_USER_ID, CONF_STRING, NULL, &ctx->conf->unauthenticated_user,
                       val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_IGNORE_PATHINFO, CONF_NUMBER, NULL, &ctx->conf->path_info_ignore, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_IGNORE_PATHINFO_NOT_ENFORCED, CONF_NUMBER, NULL,
                       &ctx->conf->path_info_ignore_not_enforced, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_PERSISTENT_COOKIE_ENABLE, CONF_NUMBER, NULL,
                       &ctx->conf->persistent_cookie_enable, val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_SCHANNEL_DISABLE, CONF_NUMBER, NULL, &ctx->conf->secure_channel_disable,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_SKIP_POST_URL, CONF_STRING_MAP, &ctx->conf->skip_post_url_map_sz,
                       &ctx->conf->skip_post_url_map, val, len);
}
...
static void end_element(void *userData, const char *name) {
    ...
    parse_config_value(ctx, AM_AGENTS_CONFIG_NAMING_URL, CONF_STRING_LIST, &ctx->conf->naming_url_sz,
                       &ctx->conf->naming_url, val, len);
    ...
    parse_config_value(ctx, AM_AGENTS_CONFIG_URL_VALIDATE_PING_INTERVAL, CONF_NUMBER, NULL, &ctx->conf->valid_ping, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_URL_VALIDATE_PING_MISS, CONF_NUMBER, NULL, &ctx->conf->valid_ping_miss,
                       val, len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_URL_VALIDATE_PING_OK, CONF_NUMBER, NULL, &ctx->conf->valid_ping_ok, val,
                       len);
    parse_config_value(ctx, AM_AGENTS_CONFIG_URL_VALIDATE_DEFAULT_SET, CONF_NUMBER_LIST,
                       &ctx->conf->valid_default_url_sz, &ctx->conf->valid_default_url, val, len);

    parse_config_value(ctx, AM_AGENTS_CONFIG_HOST_MAP, CONF_STRING_LIST, &ctx->conf->hostmap_sz, &ctx->conf->hostmap,
                       val, len);
    ...
    parse_config_value(ctx, AM_AGENTS_CONFIG_KEEPALIVE_DISABLE, CONF_NUMBER, NULL, &ctx->conf->keepalive_disable, val,
                       len);
...
static void character_data(void *userData, const char *val, int len) {
    char *tmp;
    am_xml_parser_ctx_t *ctx = (am_xml_parser_ctx_t *)userData;
    if (!ISVALID(ctx->current_name) || ctx->setting_value == 0 || len <= 0 || strncmp(val, "[]=", len) == 0 ||
        strncmp(val, "[0]=", len) == 0)
        return;
