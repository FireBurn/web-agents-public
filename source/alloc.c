...
/*
 * free list choice: 4 lists, returns 3, 2, 1, 0 depending on whether size > 3072, > 2048, > 1024, or smaller
 * (respectively)
 *
 */
...
static void reset_ctlblock(void *cbdata, void *p) {
    ctl_header_t *ctl = p;
    *ctl = (ctl_header_t){.seed = 0, .checker = 0, .error = 0, .cluster_capacity = 0, .number_of_clusters = 0};
}
...
int agent_memory_initialise(uint32_t sz, int id) {
    ...
    rv = get_memory_segment(&ctlblock_pool, CTLFILE, sizeof(ctl_header_t), reset_ctlblock, NULL, id);
    if (rv != AM_SUCCESS)
        return rv;
    ...
    rv = get_memory_segment(&cluster_hdrs_pool, HEADERFILE, sizeof(cluster_header_t) * CLUSTERS, reset_headers, NULL,
                            id);
    if (rv != AM_SUCCESS)
        return rv;
...
/*
 * try allocate within a cluster but perform cluster-wide reorganisation of freelists if allocation fails, then try
 * again
 *
 */
...
static int validate_cluster_format(unsigned cluster) {
    ...
    for (ofs = base; ofs != end; ofs += HDR(ofs)->size) {
        if (ofs < base || end < ofs) {
            AM_LOG_DEBUG(0, "%s block validation: cluster %u block range error after %d: %d", thisfunc, cluster, base,
                         ofs);
            err = 1;
    ...
    if (err == 0) {
        ...
        for (freelist_offset = 0; freelist_offset < CLUSTER_FREELISTS; freelist_offset++) {
            ...
            for (ofs = cluster_free_lists(cluster)[freelist_offset]; ~ofs; ofs = HDR(ofs)->u.free.n) {
                ...
                if (visits[v]) {
                    AM_LOG_DEBUG(0, "%s block validation: cluster %u, free list %u: cycle detected", thisfunc, cluster,
                                 freelist_offset);
                    err = 1;
                ...
                if (HDR(ofs)->u.free.p != prior) {
                    AM_LOG_DEBUG(0, "%s block validation: cluster %u, unexepected prior offset: %u", thisfunc, cluster,
                                 HDR(ofs)->u.free.p);
                    err = 1;
    ...
    for (unsigned cluster = 0; cluster < ctlblock->number_of_clusters; cluster++) {
        ...
        if (locker == 0) {
            ...
            if (cas(&cluster_lock(cluster), locker, VALIDATION_LOCK)) {
                AM_LOG_DEBUG(0, "%s cluster %u: validating: locking process %" PR_L64 " is dead", thisfunc, cluster,
                             (int64_t)locker);
 ...
 *
 */
static void analyse_cluster(int cluster, uint32_t *use_ptr, uint32_t *free_ptr, uint32_t *block_ptr,
                            uint32_t *freelists) {
    ...
    AM_LOG_DEBUG(0, "%s cluster %5u: used %8u, free %8u, blocks %5u locks types [%5u, %5u, %5u, %5u, (%u)]", thisfunc,
                 cluster, used, free, blocks, locks[0], locks[1], locks[2], locks[3], overflows);
...
void agent_memory_print(pid_t pid) {
    ...
    AM_LOG_DEBUG(0, "%s avg blocks per cluster %f, blocks in use %u, free %u", thisfunc,
                 (float)blocks / ctlblock->number_of_clusters, used, free);
    for (int x = 0; x < CLUSTER_FREELISTS; x++) {
