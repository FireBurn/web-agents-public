...
static uint64_t verify_freelists(struct mem_pool *pool, char *action) {
    ...
    while (hdr_offset < 3) {
        ...
        for (i = pool->freelist_hdrs[hdr_offset]; i != FREELIST_END;
             i = FREELIST_FROM_CHUNK(AM_GET_POINTER(pool, i))->next) {
...
int am_shm_lock(am_shm_t *am) {
    ...
    if (am->local_size != *(am->global_size)) {
        ...
        am->h[2] = CreateFileMappingA(am->h[1], sec, PAGE_READWRITE, (DWORD)((global_size >> 32) & 0xFFFFFFFFul),
                                      (DWORD)(global_size & 0xFFFFFFFFul), NULL);
        am->error = GetLastError();
        if (am->h[2] == NULL) {
            return AM_EFAULT;
        }
...
am_shm_t *am_shm_create(const char *name, uint64_t usize, int use_new_initialiser, uint64_t *limit, int *ret_status) {
    ...
    if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
        ...
        PathRemoveFileSpecA(dll_path);
        strcat(dll_path, FILE_PATH_SEP);
        snprintf(ret->name[0], sizeof(ret->name[0]), AM_GLOBAL_PREFIX "%s_l", name); /* mutex/semaphore */
        snprintf(ret->name[1], sizeof(ret->name[1]), AM_GLOBAL_PREFIX "%s_s", name); /* shared memory name */
        ...
        snprintf(ret->name[3], sizeof(ret->name[3]), AM_GLOBAL_PREFIX "%s_sz",
                 name); /* shared memory name for global_size */
    } else {
        ret->error = AM_NOT_FOUND;
        return ret;
    }
#else
    snprintf(ret->name[0], sizeof(ret->name[0]), "/%s_l", name); /* mutex/semaphore */
    ...
    ret->h[1] =
        CreateFileA(ret->name[2], GENERIC_WRITE | GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
    ...
    if (ret->h[1] == INVALID_HANDLE_VALUE && error == ERROR_FILE_EXISTS) {
        ret->h[1] = CreateFileA(ret->name[2], GENERIC_WRITE | GENERIC_READ,
                                FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, sec, OPEN_EXISTING,
    ...
    if (!opened) {
        ret->h[2] = CreateFileMappingA(ret->h[1], sec, PAGE_READWRITE, (DWORD)((size >> 32) & 0xFFFFFFFFul),
                                       (DWORD)(size & 0xFFFFFFFFul), ret->name[1]);
        error = GetLastError();
    } else {
    ...
    ret->h[3] = CreateFileMappingA(INVALID_HANDLE_VALUE, sec, PAGE_READWRITE, 0, (DWORD)sizeof(uint64_t), ret->name[3]);
    if (ret->h[3] == NULL) {
    ...
    *(ret->global_size) = ret->local_size = size;
    ...
    ret->lock = mmap(NULL, sizeof(pthread_mutex_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
    ...
    ret->global_size = mmap(NULL, sizeof(uint64_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
...
static int am_shm_extend(am_shm_t *am, uint64_t usize) {
    ...
    am->h[2] = CreateFileMappingA(am->h[1], sec, PAGE_READWRITE, (DWORD)((size >> 32) & 0xFFFFFFFFul),
                                  (DWORD)(size & 0xFFFFFFFFul), NULL);
    am->error = GetLastError();
    if (am->h[2] == NULL) {
        return AM_ERROR;
    }
...
void *am_shm_alloc_with_gc(am_shm_t *am, uint64_t usize, int (*gc)(unsigned long), unsigned long id) {
    ...
    if (usize == 0 || am == NULL || am->pool == NULL || am_shm_lock(am) != AM_SUCCESS) {
        return NULL;
    }
...
void am_shm_free(am_shm_t *am, void *ptr) {
    ...
    if (am == NULL || am->pool == NULL || ptr == NULL || am_shm_lock(am) != AM_SUCCESS) {
        return;
    }
...
void am_shm_info(am_shm_t *am) {
    ...
    AM_OFFSET_LIST_FOR_EACH(pool, head, e, t, struct mem_chunk) {
        fprintf(stdout, "CHUNK #%03d: %s  (size: %llu, user: %llu bytes) [P:%d][O:%d][N:%d]\n", ++i,
                e->used ? "used" : "free", (unsigned long long)e->size, (unsigned long long)e->usize, e->lh.prev,
...
uint64_t get_total_system_memory() {
    ...
    return (uint64_t)(sysconf(_SC_PHYS_PAGES) * sysconf(_SC_PAGE_SIZE));
#elif defined(__APPLE__)
