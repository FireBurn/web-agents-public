...
static struct ssl_func ssl_sw[] = {{"SSL_library_init", NULL},
                                   {"SSL_CTX_new", NULL},
...
#define SSL_CTX_set_msg_callback                                                                                       \
    (*(void (*)(SSL_CTX *, void (*callback)(int, int, int, const void *, size_t, SSL *, void *)))ssl_sw[14].ptr)
...
#define CRYPTO_set_mem_functions                                                                                       \
    (*(int (*)(void *(*m)(size_t), void *(*r)(void *, size_t), void (*f)(void *)))crypto_sw[3].ptr)
...
static void *load_library(const char *lib, struct ssl_func *sw) {
    ...
    int i;
    const char *name_variants[] = {"%s.so.10",    "%s.so.1.1",    "%s.so.1.0.2", "%s.so.1.0.1", "%s.so.1.0.0",
                                   "%s.so.0.9.8", "%s.so.0.9.8e", "%s.so.6",     NULL};
...
static
    ...
    ADD_DLL_PROC add_directory = (ADD_DLL_PROC)GetProcAddress(GetModuleHandleA("kernel32.dll"), "AddDllDirectory");
    ...
    if (ssl_lib != NULL && crypto_lib != NULL && CRYPTO_set_mem_functions && SSL_library_init &&
        SSL_load_error_strings && CRYPTO_num_locks && CRYPTO_set_id_callback && CRYPTO_set_locking_callback &&
        ...
        if (ssl_lib != NULL)
            close_library(ssl_lib);
        if (crypto_lib != NULL)
            close_library(crypto_lib);
        ssl_lib = NULL;
        crypto_lib = NULL;
    }
#ifdef _WIN32
...
void net_shutdown_ssl() {
    int i;
    if (SSL_library_init && CRYPTO_set_locking_callback && CRYPTO_set_id_callback && CRYPTO_num_locks) {
        CRYPTO_set_locking_callback(NULL);
        CRYPTO_set_id_callback(NULL);
        if (ssl_mutexes != NULL) {
            for (i = 0; i < CRYPTO_num_locks(); i++) {
...
void net_connect_ssl(am_net_t *n) {
    ...
    if (n != NULL) {
        ...
        if (SSL_CTX_new == NULL || SSLv23_client_method == NULL || SSL_CTX_set_msg_callback == NULL ||
            SSL_CTX_ctrl == NULL || BIO_new == NULL || BIO_s_mem == NULL || SSL_set_bio == NULL ||
            SSL_set_connect_state == NULL || SSL_do_handshake == NULL || SSL_new == NULL || SSL_get_error == NULL) {
...
void net_write_ssl(am_net_t *n) {
    ...
    do {
        ret = SSL_write(n->ssl.ssl_handle, n->ssl.request_data + written, (int)n->ssl.request_data_sz - written);
        if (ret == 0) {
            /* connection closed */
            break;
        }
