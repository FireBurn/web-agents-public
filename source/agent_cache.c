 ...
 *
 */
static void purge_identical_entries(pid_t pid, uint32_t hash, struct cache_entry *e, int i, void *data,
 ...
 *
 * NOTE: this could be done in multiple threads, but the performance problem issue would only be the dispersed memory
 * access
 *
 */
 ...
 *
 */
int cache_get_readlocked_ptr(uint32_t h, void **addr, uint32_t *ln, void *data, int64_t now,
                             int (*identity)(void *, void *)) {
    pid_t pid = getpid();
...
static int cache_garbage_checker(void *cbdata, pid_t pid, int32_t type, void *p) {
    ...
    case CACHE:
        ...
        if (hash != ~((struct cache_entry *)p)->check) {
            if (((struct cache_entry *)p)->gcdata ==
                (hash ^ GC_MARKER)) { /* as above: wait until its been seen before in this state */
                return 1;
            }
