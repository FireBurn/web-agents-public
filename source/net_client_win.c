...
static int net_write(am_net_t *net, const char *buf, int len) {
    ...
    int sent = 0, rv, error;
    int timeout = net->options && net->options->net_timeout > AM_NET_POOL_TIMEOUT ? net->options->net_timeout
                                                                                  : AM_NET_POOL_TIMEOUT;
...
void wnet_close_ssl(am_net_t *net) {
    ...
    if (n->connected) {
        ...
        if (sspi_ret != SEC_E_OK) {
            AM_LOG_ERROR(net->instance_id, "%s ApplyControlToken failed (0x%lx)", thisfunc, sspi_ret);
        }

        init_sec_buffer(&outbuf, SECBUFFER_EMPTY, NULL, 0);
        init_sec_buffer_desc(&outbuf_desc, &outbuf, 1);

        sspi_ret = InitializeSecurityContext(&n->cred, &n->ctxt, net->uv.host, n->request_flags, 0, 0, NULL, 0,
                                             &n->ctxt, &outbuf_desc, &n->context_flags, &n->ctxt_timestamp);
    ...
    if (n->cert_ctxt)
        CertFreeCertificateContext(n->cert_ctxt);
    n->cert_ctxt = NULL;
    if (n->cert_store)
...
static int net_write_ssl(am_net_t *net, const char *buf, int len) {
    ...
    if (n->sizes.cbMaximumMessage == 0) {
        ...
        if (sspi_ret != SEC_E_OK) {
            AM_LOG_ERROR(net->instance_id, "%s query context attributes failed (0x%lx)", thisfunc, sspi_ret);
            return AM_EFAULT;
        }
    }
    ...
    init_sec_buffer(&outbuf[0], SECBUFFER_STREAM_HEADER, data, n->sizes.cbHeader);
    init_sec_buffer(&outbuf[1], SECBUFFER_DATA, data + n->sizes.cbHeader, len);
    init_sec_buffer(&outbuf[2], SECBUFFER_STREAM_TRAILER, data + n->sizes.cbHeader + len, n->sizes.cbTrailer);
    ...
    do {
        ...
        if (sspi_ret == SEC_E_OK) {
            ...
            if (ret < 0 || ret != len) {
                ret = AM_EFAULT;
                AM_LOG_ERROR(net->instance_id, "%s error writing encrypted data to socket", thisfunc);
            }
            break;
        }

        AM_LOG_ERROR(net->instance_id, "%s data encryption failed (0x%lx)", thisfunc, sspi_ret);
...
#define HANDSHAKE_CLEANUP_RETURN(s)                                                                                    \
    do {                                                                                                               \
        int j;                                                                                                         \
        for (j = 0; j < 3; j++) {                                                                                      \
    ...
    while (1) {
        ...
        if (read_data) {
            ret = net_read(net, n->enc_buf + n->enc_buf_offset, n->enc_buf_size - n->enc_buf_offset);
            if (ret < 0) {
        ...
        sspi_ret = InitializeSecurityContextA(&n->cred, &n->ctxt, net->uv.host, n->request_flags, 0, 0, &inbuf_desc, 0,
                                              NULL, &outbuf_desc, &n->context_flags, &n->ctxt_timestamp);
        ...
        if (sspi_ret == SEC_I_CONTINUE_NEEDED || sspi_ret == SEC_E_OK) {
            for (i = 0; i < 3; i++) {
                if (outbuf[i].BufferType == SECBUFFER_TOKEN && outbuf[i].cbBuffer > 0) {
                    ...
                    if (ret < 0 || ret != outbuf[i].cbBuffer) {
                        AM_LOG_ERROR(net->instance_id, "%s failed to send handshake data", thisfunc);
                        HANDSHAKE_CLEANUP_RETURN(AM_EFAULT);
                    }
                }
            ...
            if (sspi_ret == SEC_E_WRONG_PRINCIPAL) {
                AM_LOG_ERROR(net->instance_id, "%s SNI or certificate check failed", thisfunc);
            } else {
                AM_LOG_ERROR(net->instance_id, "%s creating security context failed (0x%lx)", thisfunc, sspi_ret);
            }
            HANDSHAKE_CLEANUP_RETURN(AM_EFAULT);
        }
        ...
        if (inbuf[1].BufferType == SECBUFFER_EXTRA && inbuf[1].cbBuffer > 0) {
            if (n->enc_buf_offset > inbuf[1].cbBuffer) {
                memmove(n->enc_buf, (n->enc_buf + n->enc_buf_offset) - inbuf[1].cbBuffer, inbuf[1].cbBuffer);
...
static int net_read_ssl(am_net_t *net, char *buf, int len) {
    ...
    while (n->enc_buf_offset > 0 && sspi_ret == SEC_E_OK && n->dec_buf_offset < (unsigned int)len) {
        ...
        case SEC_I_CONTEXT_EXPIRED:
            ...
            if (inbuf[3].BufferType == SECBUFFER_EXTRA && inbuf[3].cbBuffer > 0) {
                if (n->enc_buf_offset > inbuf[3].cbBuffer) {
                    memmove(n->enc_buf, (n->enc_buf + n->enc_buf_offset) - inbuf[3].cbBuffer, inbuf[3].cbBuffer);
            ...
            if (sspi_ret == SEC_I_RENEGOTIATE) {
                if (n->enc_buf_offset) {
                    AM_LOG_DEBUG(net->instance_id, "%s cannot renegotiate, encrypted data buffer not empty", thisfunc);
        ...
        default:
            AM_LOG_ERROR(net->instance_id, "%s unable to decrypt message (0x%lx)", thisfunc, sspi_ret);
            return net_read_ssl_cleanup(n, buf, len, AM_EFAULT);
        }
    }
...
int wnet_write(am_net_t *net, const char *buf, int len) {
    ...
    return net->uv.ssl && n->connected ? net_write_ssl(net, buf, len) : net_write(net, buf, len);
}
...
void wnet_read(am_net_t *net) {
    ...
    do {
        rv = net->uv.ssl && n->connected ? net_read_ssl(net, buf, read_buf_sz) : net_read(net, buf, read_buf_sz);
        if (rv == AM_EAGAIN)
            continue;
        if (rv < 0) {
            if (net->on_close)
                net->on_close(net->data, 0);
            break;
        }

        http_parser_execute(net->hp, net->hs, buf, rv);
        if (rv == 0 || (net_ssl_pending(n) <= 0 && net_data_avail(net) <= 0)) {
            if (!net->is_complete(net->data))
                continue;
            if (net->on_close)
                net->on_close(net->data, 0);
            break;
...
static int net_client_handshake(am_net_t *net) {
    ...
    n->request_flags = ISC_REQ_SEQUENCE_DETECT | ISC_REQ_REPLAY_DETECT | ISC_REQ_CONFIDENTIALITY |
                       ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_STREAM | ISC_RET_EXTENDED_ERROR;

    sspi_ret = InitializeSecurityContextA(&n->cred, NULL, net->uv.host, n->request_flags, 0, 0, NULL, 0, &n->ctxt,
                                          &outbuf_desc, &n->context_flags, &n->ctxt_timestamp);

    if (sspi_ret != SEC_I_CONTINUE_NEEDED) {
        AM_LOG_ERROR(net->instance_id, "%s unable to create initial security context (0x%lx)", thisfunc, sspi_ret);
        DeleteSecurityContext(&n->ctxt);
        return AM_ENOSSL;
    }
    ...
    FreeContextBuffer(outbuf.pvBuffer);
    if (ret < 0 || ret != outbuf.cbBuffer) {
        AM_LOG_ERROR(net->instance_id, "%s failed to send initial handshake data", thisfunc);
        DeleteSecurityContext(&n->ctxt);
        return AM_EFAULT;
    }
...
static int is_ca_cert(PCCERT_CONTEXT ctxt) {
    BYTE key_usage;
    return (CertGetIntendedKeyUsage(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, ctxt->pCertInfo, &key_usage, 1) &&
            (key_usage & CERT_KEY_CERT_SIGN_KEY_USAGE));
}
...
static PCCERT_CONTEXT net_ssl_init_creds(am_net_t *net, const char *cert_file, const char *cert_pass) {
    ...
    if (ISVALID(cert_pass)) {
        ...
        if (n->cert_store != NULL) {
            cert_ctxt = CertFindCertificateInStore(n->cert_store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0,
                                                   CERT_FIND_PROPERTY, &prop_id, NULL);
            if (cert_ctxt == NULL) {
                CertCloseStore(n->cert_store, 0);
                if (blob.pbData)
                    UnmapViewOfFile(blob.pbData);
    ...
    while ((cert_ctxt = CertEnumCertificatesInStore(n->cert_store, cert_ctxt)) != NULL) {
        if (!CertGetCertificateContextProperty(cert_ctxt, CERT_FRIENDLY_NAME_PROP_ID, NULL, &size)) {
            continue;
        }
        ...
        if (!CertGetCertificateContextProperty(cert_ctxt, CERT_FRIENDLY_NAME_PROP_ID, friendly_name, &size)) {
            free(friendly_name);
            continue;
        }
    ...
    if (cert_ctxt == NULL) {
        AM_LOG_WARNING(net->instance_id, "%s unable to locate certificate by friendly name \"%s\"", thisfunc,
                       cert_file);
        return NULL;
    }
...
static void net_display_cert_chain(am_net_t *net, PCCERT_CONTEXT cert_ctxt) {
    ...
    case 0:
        ...
        break;
    default:
        time_valid = "can't validate certificate time";
        break;
    }
    ...
    while (current_cert != NULL) {
        verify_flags = 0;
        issuer_cert = CertGetIssuerCertificateFromStore(cert_ctxt->hCertStore, current_cert, NULL, &verify_flags);
        if (issuer_cert == NULL) {
...
static const char *net_cert_verify_status(DWORD status) {
    ...
    case CERT_E_UNTRUSTEDROOT:
        return "A certification chain processed correctly but terminated in a root certificate that is not trusted by "
               "the trust provider";
    ...
    case CERT_E_INVALID_NAME:
        return "The certificate has an invalid name";
    ...
    case CERT_E_PURPOSE:
        return "The certificate is being used for a purpose other than the purposes specified by its CA";
    case CERT_E_ROLE:
...
static DWORD net_verify_server_certificate(am_net_t *net, PCCERT_CONTEXT server_cert_handle, DWORD cert_flags) {
    ...
    DWORD status = SEC_E_OK, cusages = sizeof(usages) / sizeof(LPSTR);
    ...
    if (server_cert_handle != NULL && ISVALID(net->uv.host)) {
        if (CertGetCertificateChain(NULL, server_cert_handle, NULL, n->cert_store, &chain_para, 0, NULL,
                                    &chain_context)) {
            ...
            if (CertVerifyCertificateChainPolicy(CERT_CHAIN_POLICY_SSL, chain_context, &policy_para, &policy_status)) {
                if (policy_status.dwError) {
                    ...
                    AM_LOG_ERROR(net->instance_id, "%s error \"%s\" (0x%lx)", thisfunc, net_cert_verify_status(status),
                                 status);
                } else {
...
static int net_connect_ssl_win(am_net_t *net) {
    ...
    schannel_cred.grbitEnabledProtocols =
        SP_PROT_SSL3_CLIENT | SP_PROT_TLS1_0_CLIENT | SP_PROT_TLS1_1_CLIENT | SP_PROT_TLS1_2_CLIENT;
    ...
    schannel_cred.dwFlags = SCH_CRED_MANUAL_CRED_VALIDATION | SCH_CRED_IGNORE_NO_REVOCATION_CHECK |
                            SCH_CRED_IGNORE_REVOCATION_OFFLINE | SCH_CRED_REVOCATION_CHECK_CHAIN_EXCLUDE_ROOT |
    ...
    sspi_ret = AcquireCredentialsHandleA(NULL, UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &schannel_cred, NULL, NULL,
                                         &n->cred, &n->cred_timestamp);
    if (sspi_ret != SEC_E_OK) {
        AM_LOG_ERROR(net->instance_id, "%s unable to acquire security credentials (0x%lx)", thisfunc, sspi_ret);
        wnet_close_ssl(net);
        return AM_ENOSSL;
    }
    ...
    sspi_ret = QueryContextAttributesA(&n->ctxt, SECPKG_ATTR_REMOTE_CERT_CONTEXT, (VOID *)&server_cert_handle);
    if (sspi_ret != SEC_E_OK) {
        AM_LOG_ERROR(net->instance_id, "%s unable to query security context attributes (0x%lx)", thisfunc, sspi_ret);
        wnet_close_ssl(net);
        return AM_ENOSSL;
    }
    ...
    if (!net->options->cert_trust) {
        ...
        if (sspi_ret != SEC_E_OK) {
            AM_LOG_ERROR(net->instance_id, "%s unable to verify server certificate (0x%lx)", thisfunc, sspi_ret);
            wnet_close_ssl(net);
...
void sync_connect_win(am_net_t *net) {
    ...
    if (net->sock == INVALID_SOCKET) {
        AM_LOG_WARNING(net->instance_id, "%s connection to %s:%d failed", thisfunc, net->uv.host, net->uv.port);
        net->error = AM_ERROR;
        return;
    }
    ...
    if (net->error == AM_SUCCESS) {
        AM_LOG_DEBUG(net->instance_id, "%s connected to %s:%d", thisfunc, net->uv.host, net->uv.port);
    } else {
        AM_LOG_ERROR(net->instance_id, "%s failed to connect to %s:%d, error: %d", thisfunc, net->uv.host, net->uv.port,
                     net->error);
    }
}
