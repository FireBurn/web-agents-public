...
static char *utf8_encode(IHttpContext *r, const wchar_t *wstr, int *inout_len) {
    ...
    int ret_len, out_len = wstr != NULL ? WideCharToMultiByte(CP_UTF8, 0, wstr, in_len, NULL, 0, NULL, NULL) : 0;
    if (out_len == 0) {
        return NULL;
    }
...
class OpenAMStoredConfig : public IHttpStoredContext {
  ...
  private:
    HRESULT GetBooleanPropertyValue(IAppHostElement *pElement, WCHAR *pszPropertyName,
        ...
        do {
            if (pElement == NULL || pszPropertyName == NULL || pException == NULL || pBoolValue == NULL) {
                hr = E_INVALIDARG;
                break;
            }
    ...
    HRESULT GetStringPropertyValue(IAppHostElement *pElement, WCHAR *pszPropertyName,
        ...
        do {
            if (pElement == NULL || pszPropertyName == NULL || pException == NULL || ppszValue == NULL) {
                hr = E_INVALIDARG;
                break;
            }
...
class OpenAMHttpUser : public IHttpUser {
  public:
    ...
    OpenAMHttpUser(PCWSTR usrn, PCWSTR usrp, PCWSTR usrpcrypted, BOOL showpass, BOOL dologon)
        : userName(usrn), userPassword(usrpcrypted), showPassword(showpass), status(FALSE), error(0) {
        HANDLE tkn = NULL;
        m_refs = 1;
        if (dologon) {
            if (usrn != NULL && usrp != NULL) {
                status = LogonUserW(usrn, NULL, usrp, LOGON32_LOGON_NETWORK, LOGON32_PROVIDER_DEFAULT, &tkn);
                error = GetLastError();
                if (status) {
...
static const char *get_server_variable(IHttpContext *ctx, unsigned long instance_id, PCSTR var) {
    ...
    if (FAILED(ctx->GetServerVariable(var, &val, &size))) {
        AM_LOG_WARNING(instance_id, "%s server variable %s is not available in HttpContext (error: %d)", thisfunc, var,
                       GetLastError());
    } else {
        AM_LOG_DEBUG(instance_id, "%s found variable %s with value [%s]", thisfunc, var, LOGEMPTY(val));
    }
    return val;
}
...
static am_status_t get_request_url(am_request_t *req) {
    ...
    char *cooked_url = utf8_encode(ctx, ctx->GetRequest()->GetRawHttpRequest()->CookedUrl.pFullUrl, &cooked_url_sz);
    if (cooked_url == NULL) {
        return AM_ENOMEM;
    }
    ...
    if (ctx->GetRequest()->GetRawHttpRequest()->pRawUrl != NULL &&
        ...
        char *raw_url =
            (char *)alloc_request(ctx, cooked_url_sz + ctx->GetRequest()->GetRawHttpRequest()->RawUrlLength + 1);
        if (raw_url == NULL) {
            return AM_ENOMEM;
        }
    ...
    if (status == AM_SUCCESS) {
        ...
        if (ISVALID(path_info) && ISVALID(script_name)) {
            ...
            if (pos == NULL) {
                AM_LOG_WARNING(req->instance_id,
                               "%s script name %s not found in path info (%s). "
                               "Could not get the path info.",
                               thisfunc, script_name, path_info);
                return status;
            }
            ...
            if (path_info_tmp != NULL) {
                ...
                AM_LOG_DEBUG(req->instance_id, "%s reconstructed path info: %s", thisfunc, path_info_tmp);
            }
        }
    }
    return status;
...
static am_status_t set_header_in_request(am_request_t *rq, const char *key, const char *value) {
    ...
    if (FAILED(hr)) {
        AM_LOG_WARNING(rq->instance_id, "%s failed to delete request header %s (%d)", thisfunc, LOGEMPTY(key),
                       hr_to_winerror(hr));
    }
    if (ISVALID(value)) {
...
#define TEMP_BUFFER_SZ (REQ_DATA_BUFF_SZ * 2)
    ...
    while (r->GetRemainingEntityBytes() != 0) {
        ...
        AM_LOG_DEBUG(rq->instance_id, "%s read: %ld, temp: %ld bytes", thisfunc, rc, tmp_sz);
        ...
        AM_LOG_DEBUG(rq->instance_id, "%s storing into: %s, temp writes: %s", thisfunc, to_file ? "file" : "memory",
                     !tmp_writes ? "done" : "working");
            ...
            if (to_file) {
                if (fd == INVALID_HANDLE_VALUE) {
                    ...
                    if (ISINVALID(rq->conf->pdp_dir)) {
                        AM_LOG_ERROR(rq->instance_id, "%s invalid POST preservation configuration", thisfunc);
                        return AM_EINVAL;
                    }
                    ...
                    fd = CreateFileA(file_name, FILE_GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                                     CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
                    if (fd == INVALID_HANDLE_VALUE) {
                        AM_LOG_ERROR(rq->instance_id, "%s unable to create POST preservation file: %s (%d)", thisfunc,
                ...
                if (!write_status) {
                    AM_LOG_ERROR(rq->instance_id, "%s unable to write to POST preservation file: %s (%d)", thisfunc,
                                 file_name, GetLastError());
                    CloseHandle(fd);
                    return AM_FILE_ERROR;
                }
    ...
    if (status == AM_SUCCESS) {
        AM_LOG_DEBUG(rq->instance_id, "%s processed %d bytes\n%s", thisfunc, read_bytes,
                     ISVALID(out) ? out : LOGEMPTY(file_name));
...
static void send_custom_data(IHttpResponse *res, const char *payload, int payload_sz, const char *cont_type,
                             int status) {
    HTTP_DATA_CHUNK dc;
    DWORD sent;
    ...
    res->SetHeader("Content-Type", cont_type, (USHORT)strlen(cont_type), TRUE);
...
class OpenAMHttpModule : public CHttpModule {
  public:
    ...
    REQUEST_NOTIFICATION_STATUS OnAsyncCompletion(IHttpContext *ctx, DWORD dwNotification, BOOL fPostNotification,
                                                  IHttpEventProvider *prov, IHttpCompletionInfo *pCompletionInfo) {
    ...
    REQUEST_NOTIFICATION_STATUS OnBeginRequest(IHttpContext *ctx, IHttpEventProvider *prov) {
        ...
        if (init_status != AM_SUCCESS) {
            AM_LOG_ERROR(0, "%s agent init for site %d failed (error: %d)", thisfunc, site->GetSiteId(), init_status);
            res->SetStatus(AM_HTTP_STATUS_500, "Internal Server Error");
            return RQ_NOTIFICATION_FINISH_REQUEST;
        }
        ...
        if (rq_conf == NULL || rv != AM_SUCCESS) {
            AM_LOG_ERROR(site->GetSiteId(), "%s failed to get agent configuration instance, error: %s", thisfunc,
                         am_strerror(rv));
            res->SetStatus(AM_HTTP_STATUS_403, "Forbidden");
            return RQ_NOTIFICATION_FINISH_REQUEST;
        }
        ...
        if (ISVALID(d.conf->client_ip_header)) {
            d.client_ip = (char *)get_server_variable(ctx, d.instance_id, d.conf->client_ip_header);
        }
        ...
        if (ISVALID(d.conf->client_hostname_header)) {
            d.client_host = (char *)get_server_variable(ctx, d.instance_id, d.conf->client_hostname_header);
        }
        ...
        if (d.is_json_url && status_code > AM_HTTP_STATUS_302) {
            char *payload = NULL;
            int payload_sz = am_asprintf(&payload, AM_JSON_TEMPLATE_DATA, am_strerror(d.status), "\"\"", status_code);
            send_custom_data(res, payload, payload_sz, "application/json", AM_HTTP_STATUS_200);
            status = RQ_NOTIFICATION_FINISH_REQUEST;
            am_free(payload);
        }

        AM_LOG_DEBUG(site->GetSiteId(), "%s exit status: %s (%d), HTTP status: %d", thisfunc, am_strerror(d.status),
    ...
    REQUEST_NOTIFICATION_STATUS OnAuthenticateRequest(IHttpContext *ctx, IAuthenticationProvider *prov) {
        ...
        if (currentUser == NULL) {
            if (userName != NULL) {
                ...
                OpenAMHttpUser *httpUser =
                    new OpenAMHttpUser(user, userPassword, userPasswordCrypted, showPassword, doLogOn);
                if (httpUser == NULL || !httpUser->GetStatus()) {
                    AM_LOG_ERROR(site->GetSiteId(),
                                 "OpenAMHttpModule(): failed (invalid Windows/AD user credentials). "
                                 "Responding with HTTP403 error (%d)",
                                 httpUser == NULL ? ERROR_OUTOFMEMORY : httpUser->GetError());
                    res->SetStatus(AM_HTTP_STATUS_403, "Forbidden");
                    return RQ_NOTIFICATION_FINISH_REQUEST;
                } else {
    ...
    REQUEST_NOTIFICATION_STATUS OnEndRequest(IHttpContext *ctx, IHttpEventProvider *pProvider) {
...
static am_status_t set_request_body(am_request_t *rq) {
    ...
    am_status_t status = AM_SUCCESS;
    HRESULT hr = S_OK;
    OpenAMHttpModule *m = rq != NULL && rq->ctx_class != NULL ? static_cast<OpenAMHttpModule *>(rq->ctx_class) : NULL;
...
#define CONTENT_LENGTH_CSZ 128
        ...
        if (m->pdp_file == INVALID_HANDLE_VALUE) {
            AM_LOG_ERROR(rq->instance_id, "%s unable to open POST preservation file: %s (%d)", thisfunc,
                         rq->post_data_fn, GetLastError());
        ...
        if (!m->pdp_file_map) {
            ...
            AM_LOG_ERROR(rq->instance_id, "%s unable to map POST preservation file: %s (%d)", thisfunc,
                         rq->post_data_fn, GetLastError());
            return AM_FILE_ERROR;
        }
        ...
        if (!m->pdp_file_data) {
            ...
            CloseHandle(m->pdp_file);
            AM_LOG_ERROR(rq->instance_id, "%s unable to map-view POST preservation file: %s (%d)", thisfunc,
                         rq->post_data_fn, GetLastError());
            return AM_FILE_ERROR;
        }
    ...
    if (status != AM_SUCCESS) {
        AM_LOG_WARNING(rq->instance_id, "%s status %s (%d)", thisfunc, am_strerror(status), hr_to_winerror(hr));
    }
    return status;
}
...
static am_status_t set_user(am_request_t *rq, const char *user) {
    ...
    IHttpContext *r = (IHttpContext *)(rq != NULL ? rq->ctx : NULL);
    OpenAMHttpModule *m = rq != NULL && rq->ctx_class != NULL ? static_cast<OpenAMHttpModule *>(rq->ctx_class) : NULL;
    if (m == NULL || r == NULL || ISINVALID(user))
        return AM_EINVAL;
    ...
    if (m->showPassword && ISVALID(m->userPasswordCrypted)) {
        AM_LOG_DEBUG(rq->instance_id, "%s will try to setup encrypted password for user \"%s\" (AUTH_PASSWORD)",
                     thisfunc, user);
    }
    return AM_SUCCESS;
}
...
static am_status_t set_custom_response(am_request_t *rq, const char *text, const char *cont_type) {
    ...
    case AM_JSON_RESPONSE: {
        ...
        case AM_PDP_DONE: {
            ...
            payload_sz = am_asprintf(&payload, AM_JSON_TEMPLATE_LOCATION_DATA, am_strerror(rq->status),
                                     rq->post_data_url, cont_type, NOTNULL(temp), AM_HTTP_STATUS_200);
            am_free(temp);
            break;
        }
        case AM_REDIRECT:
        case AM_INTERNAL_REDIRECT:
            payload_sz =
                am_asprintf(&payload, AM_JSON_TEMPLATE_LOCATION, am_strerror(rq->status), text, AM_HTTP_STATUS_302);
            if (is_http_status(rq->conf->json_url_response_code)) {
                send_custom_data(r->GetResponse(), payload, payload_sz, "application/json",
                                 rq->conf->json_url_response_code);
            } else {
                ...
                send_custom_data(r->GetResponse(), payload, payload_sz, "application/json", AM_HTTP_STATUS_403);
            }
            rq->status = AM_DONE;
            am_free(payload);
            return AM_SUCCESS;
        default: {
            ...
            payload_sz = am_asprintf(&payload, AM_JSON_TEMPLATE_DATA, am_strerror(rq->status),
                                     ISVALID(temp) ? temp : "\"\"", AM_HTTP_STATUS_200);
            am_free(temp);
            break;
        }
        }
    ...
    case AM_REDIRECT: {
        ...
        if (FAILED(hr)) {
            AM_LOG_ERROR(rq->instance_id, "set_custom_response(): failed to issue a redirect to %s (%d)", text,
                         hr_to_winerror(hr));
            rq->status = AM_ERROR;
            break;
        }
    ...
    case AM_PDP_DONE: {
        ...
        if (_stricmp(cont_type, "application/x-www-form-urlencoded") == 0) {
            ...
            if (m->pdp_file_data) {
                ...
                for (pair = strtok_s(a, "&", &last); pair; pair = strtok_s(NULL, "&", &last)) {
                    UrlUnescapeA(pair, NULL, NULL, URL_UNESCAPE_INPLACE);
                    eq = strchr(pair, '=');
                    if (eq) {
                        *eq++ = 0;
                        form_sz =
                            am_asprintf(&form, "%s<input type=\"hidden\" name=\"%s\" value=\"%s\"/>", form, pair, eq);
                    } else {
                        form_sz = am_asprintf(&form, "%s<input type=\"hidden\" name=\"%s\" value=\"\"/>", form, pair);
                    }
                }
            }
        ...
        hr = r->GetRequest()->SetHeader("Content-Type", cont_type, (USHORT)strlen(cont_type), TRUE);
        if (FAILED(hr)) {
        ...
        hr = r->GetRequest()->SetUrl(rq->post_data_url, (DWORD)strlen(rq->post_data_url), TRUE);
        if (FAILED(hr)) {
        ...
        hr = sr->GetRequest()->SetUrl(rq->post_data_url, (DWORD)strlen(rq->post_data_url), TRUE);
        if (FAILED(hr)) {
            AM_LOG_ERROR(rq->instance_id, "set_custom_response(): SetUrl failed for %s (%d)",
                         LOGEMPTY(rq->post_data_url), hr_to_winerror(hr));
            rq->status = AM_ERROR;
        ...
        hr = r->ExecuteRequest(FALSE, sr, EXECUTE_FLAG_BUFFER_RESPONSE, NULL, &completion_expected);
        if (FAILED(hr)) {
    ...
    default: {
        ...
        if (ISVALID(cont_type)) {
            hr = r->GetResponse()->SetHeader("Content-Type", cont_type, (USHORT)strlen(cont_type), TRUE);
        }
    ...
    AM_LOG_DEBUG(rq->instance_id, "set_custom_response(): status: %s (exit: %s)", am_strerror(status),
                 am_strerror(rq->status));
    return AM_SUCCESS;
}
...
HRESULT __stdcall RegisterModule(DWORD dwServerVersion, IHttpModuleRegistrationInfo *pModuleInfo,
    ...
    do {
        ...
        status =
            pModuleInfo->SetRequestNotifications(modf, RQ_BEGIN_REQUEST | RQ_AUTHENTICATE_REQUEST | RQ_END_REQUEST, 0);
        if (FAILED(status)) {
            break;
        }
        status = pModuleInfo->SetPriorityForRequestNotification(RQ_BEGIN_REQUEST, PRIORITY_ALIAS_HIGH);
        if (FAILED(status)) {
            break;
        }
