...
void am_test_get_state_funcs(am_state_func_t const **func_array_p,
                             int *func_array_len_p);
...
static am_status_t set_custom_response(struct am_request *request,
                                       const char *data,
...
void test_session_notification_on_policy_cache(void **state) {
  ...
  assert_int_equal(
      am_add_session_policy_cache_entry(&request, session_id, result, NULL),
      AM_SUCCESS);

  delete_am_policy_result_list(&result);

  /* find the session */
  assert_int_equal(am_get_session_policy_cache_entry(&request, session_id, &r,
                                                     &session, &ets),
                   AM_SUCCESS);
...
void test_resource_notification_on_policy_cache(void **state) {
  ...
  assert_int_equal(
      am_add_session_policy_cache_entry(&request, session_id, result, 0),
      AM_SUCCESS);
  delete_am_policy_result_list(&result);
