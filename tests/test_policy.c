...
#define save_backtrack(s)                                                                                              \
    ...
    while (*u) {
        if (*p == '*') {
            p += 1;
            save_backtrack(multilevel);

        } else if (*p == '-' && p[1] == '*' && p[2] == '-') {
            p += 3;
            save_backtrack(onelevel);
        }
...
void test_am_policy_results(void **state) {
    ...
    match_wildcard("http://www.google.com:80/asdf/hello/blah/wibble/asdf/blah",
                   "http://www.google.com:80/*/blah/wibble/*/blah");
