...
static am_status_t set_custom_response(am_request_t *rq, const char *text, const char *cont_type) {
    ...
    case AM_PDP_DONE: {
        printf("set_custom_response(): issuing sub-request %s to %s (%s)", am_method_num_to_str(rq->method),
               rq->post_data_url, LOGEMPTY(cont_type));
    ...
    default: {
        printf("set_custom_response(): setting content %s to %s (%s): %s\n", am_method_num_to_str(rq->method),
               rq->post_data_url, LOGEMPTY(cont_type), text);
    } break;
    }
...
void test_handle_exits_with_success(void **state) {
    ...
    struct cookie_ctx ctx = {.in = {.c = 0}, .out = {.c = 0}};
